...
import { faSearch, faFilePdf, faSun, faMoon } from '@fortawesome/free-solid-svg-icons';
...
const RegisteredUsers = () => {
  ...
  const fetchRegisteredUsers = async () => {
    try {
      const res = await axios.get('http://localhost:5000/api/login/all-registered-users');
      setUsers(res.data);
    } catch (err) {
  ...
  const exportPDF = () => {
    const doc = new jsPDF();
    autoTable(doc, {
      head: [['ID', 'Username', 'Name', 'Email', 'Role', 'Position', 'Active', 'Registered At']],
  ...
  return (
    ...
    >
      <div className="d-flex justify-content-between align-items-center mb-3">
        <h2 className="mb-0">Registered Employees</h2>
